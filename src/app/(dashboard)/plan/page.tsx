...
import { COLORS, DAYS, PLATFORMS } from '@/constants/theme';
  ...
  const handleAddItem = async () => {
    ...
    const { data } = await supabase
      ...
      .insert({
        plan_id: planId,
        day: addItem.day,
        platform: addItem.platform,
        content_type: addItem.content_type,
        title: addItem.title,
      ...
      {weekItems.length === 0 && !generating ? (
        <>
          <Card style={{ marginBottom: '16px', textAlign: 'center', padding: '40px 24px' }}>
            ...
            <h2 className="heading-lg" style={{ fontWeight: 600, color: 'var(--c-text)', margin: '0 0 8px 0' }}>Plan your week</h2>
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
            <div>
              <h2 className="heading-lg" style={{ fontWeight: 600, color: 'var(--c-text)', margin: 0 }}>{weekTitle || 'Your Plan'}</h2>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {planId && (
                <Button variant="ghost" size="sm" onClick={handleDeletePlan} style={{ color: 'var(--c-error)' }}><Trash2 size={16} /> Clear</Button>
              )}
            </div>
          </div>

          <Card style={{ marginBottom: '20px' }}>
            <WeeklyProgress completed={completedCount} total={weekItems.length} />
          </Card>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Content Item">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '14px', fontWeight: 500, color: 'var(--c-text)' }}>Day</label>
