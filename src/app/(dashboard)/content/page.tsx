...
import { PLATFORMS, BORDER_RADIUS } from '@/constants/theme';
  ...
  const handleSave = async () => {
    ...
    if (editingItem) {
      await supabase.from('content_ideas').update({
        title: form.title, description: form.description, platform: form.platform, content_type: form.content_type,
      }).eq('id', editingItem.id);
    } else {
      await supabase.from('content_ideas').insert({
        user_id: profile.id, title: form.title, description: form.description,
        platform: form.platform, content_type: form.content_type, status: 'idea',
      });
    }
    setShowAdd(false);
    setEditingItem(null);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--c-text)', margin: 0 }}>Content</h1>
          <p style={{ fontSize: '14px', color: 'var(--c-text-secondary)', margin: '4px 0 0 0' }}>
            {items.length} ideas saved
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', overflowX: 'auto', paddingBottom: '4px' }}>
        <button onClick={() => setFilter('all')} style={{
          ...
          backgroundColor: filter === 'all' ? 'var(--c-text)' : 'var(--c-surface)',
          color: filter === 'all' ? 'var(--c-bg)' : 'var(--c-text)',
        }}>All</button>
        {PLATFORMS.map((p) => (
          <button key={p.id} onClick={() => setFilter(p.id as Platform)} style={{
            ...
            color: filter === p.id ? '#FFFFFF' : 'var(--c-text)',
          }}>{p.label}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Add your first content idea to get started"
          action={<Button size="sm" onClick={() => setShowAdd(true)}><Plus size={16} /> Add Idea</Button>}
        />
      ) : (
          ...
          {filtered.map((item) => (
            <div key={item.id} style={{ position: 'relative' }}>
              <ContentCard
                ...
                platform={item.platform} contentType={item.content_type} status={item.status as any}
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setEditingItem(null); }} title={editingItem ? 'Edit Idea' : 'Add Content Idea'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '14px', fontWeight: 500, color: 'var(--c-text)' }}>Platform</label>
            <select value={form.platform} onChange={(e) => setForm(p => ({ ...p, platform: e.target.value as Platform }))}
              style={{ padding: '12px 16px', borderRadius: '12px', border: '1.5px solid var(--c-border)', backgroundColor: 'var(--c-surface)', color: 'var(--c-text)', fontSize: '16px', outline: 'none', cursor: 'pointer' }}>
              {PLATFORMS.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}
            </select>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '14px', fontWeight: 500, color: 'var(--c-text)' }}>Content Type</label>
